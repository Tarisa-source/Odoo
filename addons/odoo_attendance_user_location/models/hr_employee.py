...
class HrEmployee(models.AbstractModel):
    ...
    def _attendance_action_change(self, longitudes, latitudes):
        ...
        office_location = (-7.793176, 110.388384)
        # office_location = [-7.7930799860944004, 110.40830481197183] testing lokasi kos
        # Ganti dengan koordinat kantor yang sesungguhnya

        def is_within_radius(coordinate1, coordinate2, radius):
            return geodesic(coordinate1, coordinate2).meters <= radius
        ...
        if self.attendance_state != 'checked_in':
            # Lokasi karyawan
            employee_location = (latitudes, longitudes)

            # radius = 1000.0
            # Periksa apakah karyawan berada dalam radius 5 meter dari kantor
            if is_within_radius(employee_location, office_location, 2500):
                vals = {
                    'employee_id': self.id,
                ...
                print(geodesic(employee_location, office_location).meters)
                # print(employee_location, office_location)
                raise exceptions.UserError(_("You are not within the allowed radius to check in."))
