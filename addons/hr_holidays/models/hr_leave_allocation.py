...
class HolidaysAllocation(models.Model):
    ...
    def _end_of_year_accrual(self):
        ...
        fifteen = today + relativedelta(months=-15, day=31)
        # Perulangan untuk setiap alokasi
        for allocation in self:
            current_level = allocation._get_current_accrual_plan_level_id(first_day_this_year)[0]
            if not current_level:
                continue
            if current_level.x_periode == 'fifteenmonth' and current_level.postpone_max_days:
                lastcall = current_level._get_previous_date(
                    fifteen) if allocation.lastcall < current_level._get_previous_date(fifteen) else allocation.lastcall
                nextcall = current_level._get_next_date(fifteen)
                i = 0
                while i <=  24:
                    next_date = today + relativedelta(months=15 * i)
                    # print(next_date)
                    if allocation.nextcall:
                        allocation.nextcall = next_date
                        allocation._process_accrual_plans(next_date, True)
            ...
            lastcall = current_level._get_previous_date(
                last_day_last_year) if allocation.lastcall < current_level._get_previous_date(
                last_day_last_year) else allocation.lastcall
            nextcall = current_level._get_next_date(last_day_last_year)
            if current_level.x_periode == 'lost':
                allocation.write(
                    {'number_of_days': allocation.leaves_taken, 'lastcall': lastcall, 'nextcall': nextcall})
            elif current_level.x_periode == 'postponed' and current_level.postpone_max_days:
                if allocation.nextcall:
        ...
        for allocation in self:
            ...
            while allocation.nextcall <= date_to:
                ...
                if allocation.lastcall.year < allocation.nextcall.year and\
                    current_level.x_periode == 'postponed' and\
