        ...
        for attendance in self:
            ...
            if not attendance.check_out:
                ...
                if last_attendance_before_check_out and last_attendance_before_check_in != last_attendance_before_check_out:
                    ...
                    # Menambahkan kondisi untuk menonaktifkan tombol check-in jika check-in dan check-out sudah dilakukan pada hari yang sama
                    today_attendances = self.env['hr.attendance'].search([
                        ('employee_id', '=', attendance.employee_id.id),
                        ('check_in', '>=', datetime.combine(attendance.check_in.date(), datetime.min.time())),
                        ('check_in', '<', datetime.combine(attendance.check_in.date(), datetime.max.time())),
                        ('id', '!=', attendance.id),
                        # Fungsi datetime.combine digunakan untuk menggabungkan tanggal
                    ])
                    if today_attendances and any(
                            today_attendance.check_out for today_attendance in today_attendances):
                        # fungsi any digunakan untuk memeriksa adakah setidaknya ada check-out di hari yang sama
                        raise exceptions.ValidationError(
                            _("Tidak bisa membuat daftar hadir baru untuk karyawan %(empl_name), karyawan tersebut sudah melakukan check-in dan check-out di hari yang sama yaitu %(date)") % {
                                'empl_name': attendance.employee_id.name,
